...
function renderModels(r,rec){
 ...
 $('modelCards').innerHTML=r.models.map(m=>{
  ...
  const credit=m.key==='pure'?r.pureClientCredit:r.regularClientCredit,partial=(m.key==='real'||m.key==='presumed')&&!m.totalComplete;
  const badge=rec.key===m.key?(partial?'★ MELHOR BASE COMPARÁVEL':'★ RECOMENDADO'):'';
  const amountLabel=partial?'Base anual comparável · sem CPP':'Saída anual estimada';
  const rateLabel=partial?'Base comparativa / faturamento':'Alíquota efetiva ampliada';
  return `<article class="modelCard ${rec.key===m.key?'recommended':''}">${badge?`<span class="badge">${badge}</span>`:''}<h3>${m.name}</h3><div class="sub">${info[m.key].sub}</div>${m.key!=='pure'?`<div class="scenarioTag">Cenário ${r.year}</div>`:''}<div class="labelMoney">${amountLabel}</div><div class="bigMoney">${brl2.format(m.total)}</div><div class="eff">${rateLabel}: ${pct1(e)}</div><div class="modelRows">${rows.map(x=>`<div><span>${x[0]}</span><b>${brl2.format(x[1])}</b></div>`).join('')}</div><div class="creditBlock"><span>Crédito potencial nas vendas B2B</span><strong>${brl2.format(credit)}</strong><small>${m.key==='pure'?'Limitado à parcela de IBS/CBS devida no Simples':'Calculado com a alíquota efetivamente incidente após a composição dos tratamentos tributários'}</small></div><div class="prosBox"><strong>✓ Vantagens</strong><ul>${info[m.key].pros.map(x=>`<li>${x}</li>`).join('')}</ul></div><div class="consBox"><strong>! Atenção</strong><ul>${info[m.key].cons.map(x=>`<li>${x}</li>`).join('')}</ul></div></article>`;
 }).join('');
}
