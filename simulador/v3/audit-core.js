...
(function(){
 ...
 window.patchAuditImportAnalyzer=function(){
  ...
  const wrapped=function(file,parsed){
   ...
   let pretax=lastFirstLineValue(text,[['lucro antes do irpj e csll'],['resultado antes do irpj e csll'],['lucro antes do imposto de renda'],['resultado antes dos tributos sobre o lucro']]),pretaxReason='Resultado antes dos tributos sobre o lucro localizado na DRE.';
   if(pretax==null&&dreDoc){const netResult=lastLineValue(text,['resultado do exercicio','lucro liquido','resultado liquido']),taxProvision=lastLineValue(text,['provisao de irpj e csll','provisao para irpj e csll']);if(netResult!=null&&taxProvision!=null){pretax=netResult+Math.abs(taxProvision);pretaxReason='Resultado do exercício somado à provisão de IRPJ e CSLL identificada na DRE.'}}
