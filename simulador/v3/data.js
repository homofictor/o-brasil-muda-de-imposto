...
// Tabelas do Simples. A sexta faixa sofre ajustes nominais na transição para alguns anexos.
const annexBands = {
...
// Repartição de CBS e IBS dentro do DAS conforme anexos da LC 214/2025. Índices em % da parcela do DAS.
const splitShares = {
...
function bandsForYear(annex,year){
  ...
  if(year<=2028){
    ...
    if(annex==='III'||annex==='IV')t[5]=[4800000,.329,648000];
...
function simplesRate(annex,rbt12,year){
  ...
  const [limit,nominal,deduction]=t[idx];const effective=rbt12>0?Math.max(0,(rbt12*nominal-deduction)/rbt12):0;
  return{effective,idx,nominal,deduction,limit};
}
function dasConsumptionShare(annex,bandIndex,year){
  const s=splitShares[annex],i=bandIndex;
  if(year<=2028)return{cbs:s.cbs27[i]/100,ibs:s.ibs27[i]/100};
  const ibsFactor={2029:.1,2030:.2,2031:.3,2032:.4,2033:1}[year]||1;
  return{cbs:s.cbs33[i]/100,ibs:(s.ibs33[i]/100)*ibsFactor};
}
...
function revenueRateFactor(){
  const ids=[['mixFull',1],['mix30',.7],['mix40',.6],['mix60',.4],['mixZero',0]];
  const total=ids.reduce((s,[id])=>s+num(id),0);const el=$('mixTotal');
  if(el){el.textContent=`${total.toFixed(0)}%`;el.className='total '+(Math.abs(total-100)<.01?'ok':'bad')}
  if(total<=0)return 1;
  return ids.reduce((s,[id,f])=>s+num(id)*f,0)/total;
}
function cnaeFmt(code){
  const s=String(code||'').replace(/\D/g,'').padStart(7,'0');if(s.length!==7)return String(code||'');return `${s.slice(0,2)}.${s.slice(2,4)}-${s.slice(4)}/${s.slice(5,7)}`;
}
...
function applyFactorR(){if(cnaeSuggestion?.factorR&&$('factorMode').value==='auto'){$('annex').value=factorRValue()>=.28?'III':'V';$('annexHint').textContent=`Fator R estimado em ${pct1(factorRValue())}. Sugestão automática: Anexo ${$('annex').value}.`}}
