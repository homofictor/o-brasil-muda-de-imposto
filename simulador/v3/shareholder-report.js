...
(function(){
 ...
 function render(){
  try{
   ...
   if(purchaseProxy)pending.push(r.operatingCreditProfile?.annualAmount>0
    ?`Revisar a base de aquisições creditáveis. A proxy da DRE agora combina custos e despesas operacionais de terceiros explicitamente identificadas, alcançando aproximadamente ${Number(purchaseProxy.value).toLocaleString('pt-BR',{maximumFractionDigits:1})}% da receita bruta. Cerca de ${money(r.operatingCreditProfile.annualAmount)} correspondem a despesas operacionais com potencial de crédito, ainda sujeitas à confirmação documental.`
    :`Revisar a base de aquisições creditáveis. A DRE indica custos equivalentes a aproximadamente ${Number(purchaseProxy.value).toLocaleString('pt-BR',{maximumFractionDigits:1})}% da receita bruta, como proxy gerencial, mas custos não equivalem necessariamente às compras creditáveis do período.`);
