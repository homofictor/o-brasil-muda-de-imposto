...
async function fetchJson(url,timeoutMs=12000){
  ...
  try{const r=await fetch(url,{headers:{Accept:'application/json'},signal:controller.signal});let payload={};try{payload=await r.json()}catch(_){payload={}}return{r,payload}}finally{clearTimeout(timer)}
}
async function queryCnpj(raw){
  let endpointError=null;
  try{const {r,payload}=await fetchJson(`/api/cnpj?cnpj=${encodeURIComponent(raw)}`);if(r.ok&&payload?.data)return payload;endpointError=new Error(payload?.error||`Endpoint respondeu ${r.status}.`)}catch(err){endpointError=err}
...
async function lookupCnpj(){
  const raw=$('cnpj').value.toUpperCase().replace(/[^A-Z0-9]/g,'');const status=$('lookupStatus');
  if(raw.length!==14){status.className='status bad';status.textContent='Informe um CNPJ com 14 posições.';return}
  status.className='status';status.textContent='Consultando dados públicos...';$('lookupBtn').disabled=true;
  try{const payload=await queryCnpj(raw);companyData=payload.data;fillCompany(companyData);status.className='status ok';status.textContent=`Dados encontrados em ${payload.provider||'base pública'}. Confirme o enquadramento antes de decidir.`}
  catch(err){status.className='status bad';status.textContent=err?.name==='AbortError'?'A consulta demorou além do esperado. Tente novamente.':(err.message||'Falha na consulta.')}
  finally{$('lookupBtn').disabled=false;calculate()}
}
function fillCompany(d){
  ...
  $('companyMei').textContent=d.opcao_pelo_mei===true?'MEI: sim':d.opcao_pelo_mei===false?'MEI: não':'MEI: não confirmado';
  $('companySize').textContent=d.porte||d.descricao_porte||'Não informado';$('companyLocation').textContent=[d.municipio,d.uf].filter(Boolean).join(' / ')||'Não informado';$('companyNature').textContent=d.natureza_juridica||'Não informado';$('companyCnae').textContent=`${cnaeFmt(d.cnae_fiscal)} · ${d.cnae_fiscal_descricao||'Descrição não informada'}`;
  $('activity').value=`${cnaeFmt(d.cnae_fiscal)} · ${d.cnae_fiscal_descricao||''}`;
  ...
  const kind=cnaeSuggestion.kind;if(kind==='service')numSet('legacyRate',5);else if(kind==='commerce'||kind==='industry')numSet('legacyRate',10);
  applyFactorR();
}
