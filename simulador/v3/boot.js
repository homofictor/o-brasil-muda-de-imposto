...
function renderNarrative(r,rec,structural,srec){
 ...
 if(rec.key==='real')parts.push(`<p><strong>Leitura tributária.</strong> O Lucro Real aparece ${cppPartial?'com a menor base comparável entre os regimes regulares':'como a alternativa de menor desembolso validado nas premissas atuais'}. Essa leitura parte do lucro contábil informado ou importado e dos ajustes fiscais lançados. A qualidade da DRE e a conferência das adições, exclusões e compensações são decisivas.${cppPartial?' A projeção total só fica completa após informar a contribuição patronal.':''}</p>`);
...
function calculate(){
 ...
 renderVatSummary(r);renderTimeline(all);renderModels(r,rec);renderCompetition(r);renderCash(r);if(typeof renderEconomicImpact==='function')renderEconomicImpact(r);renderActions(r,rec);renderNarrative(r,rec,structural,srec);if(typeof refreshMoneyInputs==='function')refreshMoneyInputs();
