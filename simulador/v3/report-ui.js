...
(function(){
 ...
 function renderConclusion(r,rec,all,srec){
  ...
  if(impact?.known){const direction=impact.resultEffect>0?'ganho':impact.resultEffect<0?'perda':'efeito neutro';economic=impact.financeCostKnown?`<p><strong>Efeito econômico.</strong> Depois da parcela transferida ao preço e do custo financeiro estimado, o cenário indica <strong>${direction} de ${money(Math.abs(impact.resultEffect))}</strong> por ano no resultado.${impact.projectedOperatingMargin==null?' A margem atual não foi informada, por isso a margem futura não foi projetada.':` A margem operacional projetada é de <strong>${pct(impact.projectedOperatingMargin)}</strong>.`}</p>`:`<p><strong>Efeito econômico parcial.</strong> Antes do custo financeiro, o cenário indica <strong>${direction} de ${money(Math.abs(impact.resultEffect))}</strong> por ano. Informe reserva e taxa financeira para concluir o impacto no resultado.</p>`}
