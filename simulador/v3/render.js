...
function renderCash(r){
 ...
 $('workingCapital').textContent=brl.format(c.working);$('workingCapitalText').textContent=c.reserveKnown?`Reposição do float e eventual retenção temporária excedente. Reserva financeira considerada: ${brl.format(c.reserve)}.`:'Reposição do float e eventual retenção temporária excedente. Informe caixa/aplicações para calcular o gap de financiamento.';$('grossSplit').textContent=brl.format(c.grossMonthly);$('floatReplacement').textContent=brl.format(c.floatReplacement);$('temporaryExcess').textContent=brl.format(c.temporaryExcess);$('financingGap').textContent=c.gap==null?'—':brl.format(c.gap);$('financingCost').textContent=c.cost==null?'—':brl.format(c.cost);$('cashPer100').textContent=brl2.format(c.cash100);
}
