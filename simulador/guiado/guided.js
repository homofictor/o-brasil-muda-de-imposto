(function(){
 ...
 function reviewRequirements(){
  ...
  if(nonSimple&&!provided('realAccountingProfitAnnual')&&!provided('projectedRealProfitAnnual'))req.push({level:'required',field:'realAccountingProfitAnnual',step:3,title:'Informar ou importar um resultado contábil de referência',why:'Sem histórico ou projeção o Lucro Real fica fora da comparação.'});
  if(nonSimple&&provided('realAccountingProfitAnnual')&&!provided('projectedRealProfitAnnual')&&value('profitProjectionMode')==='sensitivity')req.push({level:'important',field:'projectedRealProfitAnnual',step:3,title:'Avaliar a lucratividade futura',why:'O resultado histórico será tratado apenas como referência. O relatório mostrará o ponto de indiferença Real x Presumido até que exista uma projeção confiável.'});
